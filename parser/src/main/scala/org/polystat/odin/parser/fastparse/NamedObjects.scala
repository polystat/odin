...
private[parser] object NamedObjects {
  ...
  def namedObject[_: P](
    ...
    namedAbstraction(indent, indentationStep) | namedApplication(
      indent,
      indentationStep
    )

  def namedApplication[_: P](
    indent: Int,
    indentationStep: Int
  ): P[EOBndExpr[EOExprOnly]] = P(
    namedInverseDotApplication(
      indent,
      indentationStep
    ) | namedRegularApplication(indent, indentationStep)
  )

  def namedRegularApplication[_: P](
    indent: Int,
    indentationStep: Int
  ): P[EOBndExpr[EOExprOnly]] = P(
  ...
  def namedInverseDotApplication[_: P](
    indent: Int,
    indentationStep: Int
  ): P[EOBndExpr[EOExprOnly]] = P(
