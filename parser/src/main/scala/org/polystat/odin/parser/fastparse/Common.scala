...
private[parser] object Common {
  ...
  def boundAttributes[_: P](indent: Int, indentationStep: Int)
  : P[Vector[EOBndExpr[EOExprOnly]]] = P(
    "\n" ~ deeper(indent, indentationStep) ~~/
      NamedObjects.namedObject(indent + indentationStep, indentationStep)
        .repX(sep = "\n" ~ deeper(indent, indentationStep)./)
  ).map(_.toVector)

  def verticalApplicationArgs[_: P](indent: Int, indentationStep: Int)
  : P[NonEmpty[EOBnd[EOExprOnly], Vector[EOBnd[EOExprOnly]]]] = P(
    "\n" ~ deeper(indent, indentationStep) ~~/
      Parser.`object`(indent + indentationStep, indentationStep)
        .repX(1, sep = "\n" ~ deeper(indent, indentationStep)./)
  ).map(createNonEmpty)
