...
/**
  ...
  *   Type of the EO code.
  * @tparam NR
  *   Type of the name in EO to which the EO code represented as `R` will be
  *   bound.
  */
...
object EOBndRepr {
  ...
  object ops {
    ...
    def bindToName[R, NR](src: R)(name: NR)(implicit
    ...
    implicit val stringEORepr: EOBndRepr[String, String] =
      new EOBndRepr[String, String] {
        ...
        override def bindToName: String => String => String =
          (src: String) =>
            (name: String) => s"$src ${Constants.SYMBS.BINDING} $name"
