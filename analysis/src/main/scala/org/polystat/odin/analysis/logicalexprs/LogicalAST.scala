...
sealed case class LObj(attrs: List[AttrBnd], phiAttr: LExpr) extends LExpr {
  private val attrProps: Term = {
    attrs match {
      case one :: Nil => one.content.properties
      case several if several.nonEmpty => And(several.map(_.content.properties))
      case _ => True()
    }
  }
  private val namesCorrespond: Term = {
    attrs match {
      case one :: Nil => one.content.properties
      case several if several.nonEmpty => And(attrs
        .map(attr => Equals(attr.identifier, attr.content.expr))
      )
      case _ => True()
    }
  }
  private lazy val declarations: List[DeclareConst] = {
