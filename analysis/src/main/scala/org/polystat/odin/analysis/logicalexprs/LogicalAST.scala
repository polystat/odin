...
abstract class BaseLObj(attrs: List[AttrBnd], val name: String) extends LExpr {
  ...
  protected def namesCorrespond(prefix: String): Term = {
    attrs.filter{
      ...
      case one :: Nil => Equals(one.identifier(prefix), one.content.expr(prefix))
      case several if several.nonEmpty => And(several
        .map(attr => Equals(attr.identifier(prefix), attr.content.expr(prefix)))
      )
      case _ => True()
    }
  }
...
final case class LObj(attrs: List[AttrBnd], phi: Option[LExpr], override val name: String) extends BaseLObj(attrs, name) {
  ...
  override def properties(prefix: String): Term =
    And(namesCorrespond(name + prefix), attrProps(name + prefix), phi.map(_.properties(name + prefix)).getOrElse(True()))
}
...
final case class LMethod(attrs: List[AttrBnd], arguments: List[Attr], phiAttr: LExpr, override val name: String) extends BaseLObj(attrs, name) {
  ...
  def adjustedArgDeclarations(prefix: String): List[DeclareConst] = arguments.map(_.declaration(prefix))

  def declarations(prefix: String): List[DeclareConst] =
    innerDeclarations(prefix) ++ adjustedArgDeclarations(prefix) ++ attrDeclarations(prefix)
...
final case class LAdd(a: LExpr, b: LExpr) extends LExpr {
  override def properties(prefix: String): Term = And(a.properties(prefix), b.properties(prefix))
...
final case class Attr(name: String, fixedPrefix: String = "") extends LExpr {
  ...
  def declaration(relativePrefix: String): DeclareConst = DeclareConst(SSymbol(relativePrefix + fixedPrefix + name), IntSort())
}
